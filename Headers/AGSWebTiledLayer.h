...
/** @brief A layer based on non-ArcGIS map tiles.
 ...
 The tiles are fetched through a template URL such as http://${subDomain}.host.com/${level}/${col}/${row}.png where level corresponds to a zoom level, and column and row represent tile column and row, respectively. The file extension may change depending upon the image format used by the service, and subdomain may not be present. This pattern is not required, but is the most common one on the web currently and used by Map Quest, Open Cycle Map, Stamen, Bing, etc.
 
 
 @define{AGSWebTiledLayer.h, ArcGIS}
 @since 10.2.3
 */

@interface AGSWebTiledLayer : AGSTiledServiceLayer <AGSSecuredResource>

/** A template URL for the tile service such as http://${subDomain}.host.com/${level}/${col}/${row}.png where level corresponds to a zoom level, and column and row represent tile column and row, respectively. The file extension may change depending upon the image format used by the service, and subDomain may not be present. This pattern is not required, but is the most common one on the web currently and used by Map Quest, Open Cycle Map, Stamen, Bing, etc. For eg. http://{subDomain}.tile.opencyclemap.org/cycle/{level}/{col}/{row}.png
...
/** Initializes a @c AGSWebTileLayer object.
