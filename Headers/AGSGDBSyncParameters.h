...
/** @brief Parameters to synchronize a sync-enabled geodatabase with its originating service
 
 Instances of this class represent the parameters that will be sent to the @c AGSGDBSyncTask#syncGeodatabase:params:status:completion: method.
...
/**
 Array of @c AGSGDBLayerSyncInfo objects specifying the direction in which individual layers in the geodatabase must sync changes with the service. Not all layers need to be included in the sync operation. Some of them can be excluded by leaving them out of the array.

 Only applies if the geodatabase uses a sync model of @c AGSGDBSyncModelPerLayer, else use #syncDirection
 @see @c AGSGDBGeodatabase#syncModel
 @agssince{10.2, 10.2}
 */
...
/**
 ...
 Defaults to AGSGDBSyncDirectionBidirectional unless @c #initWithGeodatabase: is used, in which case, 
 defaults best suited to the given geodatabase are chosen.
 @see @c #layerSyncInfos to specify sync direction on a layer-by-layer basis.
 @agssince{10.2, 10.2}
