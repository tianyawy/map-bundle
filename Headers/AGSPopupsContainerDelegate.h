...
/** Tells the delegate that a user is ready to edit the geometry for
 ...
 if you are using an @c AGSSketchGraphicsLayer to allow the user to interactively reshape the feature, this will be the same geometry that
 you give the AGSSketchGraphicsLayer. Another option is that you could update the geometry based on the GPS coordinates reported by @c AGSLocationDisplay.
 @param popupsContainer The popup view controller
...
/** The method called by the AGSPopupsContainer when the user is done editing the current popup.
 @note If the popup is for an online feature (represented by @c AGSGraphic objects belonging to @c AGSFeatureLayer), the popupsContainer only applies edits to the feature in memory. You are responsible for applying the edit on the server or persisting it some other way to ensure it does not get lost when the application terminates.
 If the popup is for an offline feature (represented by @c AGSGDBFeature in a local geodatabase), the popupsContainer automatically persists edits to the geodatabase on the device. You only need to sync changes in the geodatabase with the server.
 @param popupsContainer The popup view controller
...
/** The method called by the AGSPopupsContainer when the user confirms they want to delete the feature.
 ...
 @note This method will only be called for online features (represented by @c AGSGraphic objects belonging to @c AGSFeatureLayer) which the developer must delete themselves. It will NOT be called for features in a local geodatabase (represented by @c AGSGDBFeature objects). Local features will be automatically deleted by the SDK and then @c #popupsContainer:didDeleteForPopup: will be called.
 @param popupsContainer The popup view controller
...
/** The method called by the AGSPopupsContainer when it successfully deletes an @c AGSGDBFeature in a local geodatabase.
 @param popupsContainer The popup view controller
...
/** The method called by the AGSPopupsContainer when it fails to delete an @c AGSGDBFeature in a local geodatabase.
