...
/** @brief Delegate to display callouts for a layer's features
 
 A protocol which must be adopted by any class wishing to control the display of callouts for a layer's features. An instance of the class must then be assigned to @c AGSHitTestable#calloutDelegate
 
 @see AGSCalloutDelegate
 @define{AGSLayerCalloutDelegate.h, ArcGIS}
 @agssince{10.2, 10.2}
 */
...
/** This method is called when a user taps on some features in a layer. Delegates should implement this method when they want to configure how a callout looks for a particular feature in a particular layer.
 Some properties delegates may want to configure are title, detail, image, customView, etc. The delegate should return YES for the callout to be shown. The delegate can return NO for those features for which a callout should not be shown. If this method is not implemented, @c AGSCallout#delegate will be consulted instead.
