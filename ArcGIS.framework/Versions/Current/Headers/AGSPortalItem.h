...
/** Kicks off an operation that fetches raw bytes for the item. The corresponding methods on @c AGSPortalItemDelegate 
...
/** Kicks off an operation that fetches the thumbnail for the item. The corresponding methods on @c AGSPortalItemDelegate 
 are invoked when the operation completes successfully or encounters an error.
...
/** Kicks off an operation that fetches the comments for the item. The corresponding methods on @c AGSPortalItemDelegate 
...
/** Kicks off an operation that fetches the groups that this item belongs to. The corresponding methods on @c AGSPortalItemDelegate 
...
/** Update the portal item's properties on the portal. The corresponding methods on @c AGSPortalItemDelegate 
 are invoked when the operation completes successfully or encounters an error.
...
/** Update the JSON data for the item on the portal. This method should be used when the context
 for this item is a web map. The corresponding methods on @c AGSPortalItemDelegate are invoked when 
...
/** Update the file for the item on the portal. This method should be used when the content for
 this item is a static file, such as a shapefile, CSV, etc. The corresponding methods on @c AGSPortalItemDelegate 
 are invoked when the operation completes successfully or encounters an error.
...
/** Add a comment to an item. Available only to authenticated users who have access to the item. The corresponding 
 methods on @c AGSPortalItemDelegate are invoked when the operation completes successfully or encounters an error.
...
/** Add a rating to an item that you have access to. Only 1 rating can be given to an item per user. 
 ...
 @c AGSPortalItemDelegate are invoked when the operation completes successfully or encounters an error.
...
/** @brief The delegate of @c AGSPortalItem
 ...
 An instance of the
 class must then be set as @c AGSPortalItem#delegate .
