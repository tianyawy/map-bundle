...
/** @brief Handles @c NSURLSession related events for background downloads
 
 This class handles @c NSURLSession related events for downloads performed while the
...
/** Provides a way to process @c NSURLSession related events for data downloaded in the background. 
 The download could have been initiated in the background by @c AGSTask#checkStatusForAllResumableTaskJobsWithCompletion:,
 ...
 Once the download finishes, the completion handler on your @c AGSGDBSyncTask or @c AGSExportTileCacheTask will be invoked to let you know that the job has completed. If your app is still in the background, you can post a local notification to alert the user.
 
 @param completionHandler The completion handler that will be called by the API when downloading is finished to let the system know that your app’s user interface is updated and a new snapshot can be taken. This handler is provided to UIApplicationDelegate by the system. Before invoking this handler, the API will first call the completion handler on your @c AGSGDBSyncTask or @c AGSExportTileCacheTask to let you know that the job has completed so that you can update your app's UI, and if desired, post a local notification to alert the user.
