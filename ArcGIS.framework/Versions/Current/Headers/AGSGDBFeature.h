...
/** Whether or not the feature exists inside of it's table yet.
 If not then you must call @c AGSGDBFeatureTable#saveFeature:error: to get it in the table.
 @since 10.2.2
 */
...
/** An array of @c AGSGDBAttachemnt objects representing attachments belonging to this feature.
 This will only contain attachments that you've added until @c #fetchAttachmentsWithCompletion: is called.
 @agssince{10.2, 10.2}
 */
-(NSArray*)attachments;
...
/** Add a new attachment for this feature. The attachment exists only in memory until it is 
 commited to the geodatabase using @c AGSFeatureTable#saveFeature:error:
...
/** Delete an existing attachment for this feature. The attachment is not deleted from the
 the geodatabase until @c AGSFeatureTable#saveFeature:error: is called.
